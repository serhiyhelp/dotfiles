...
def remove_duplicate_nodes(elem,
                           minlength, maxdist, maxdist2,
                           allowReverse, optionJoin):
                ...
                while proceed and i + e + 1 <= len(sub) - 1:
                    ...
                    if nextlength >= minlength:  # not include the next segment
                        ...
                        if lastCombined == False and i > 1:
                            ...
                            new[-1][-1][1][0] = 0.5*(
                                new[-1][-1][1][0]
                                + sub[i+e][1][0])  # change position to average
                            new[-1][-1][1][1] = 0.5*(
                                new[-1][-1][1][1]
                                + sub[i+e][1][1])
                            ...
                            if e > 0:
                                ...
                                new[-1][-1][1][0] = 0.5*(
                                    new[-1][-1][1][0]
                                    + sub[i+e-1][1][0])  # change position to average first/last
                                new[-1][-1][1][1] = 0.5*(
                                    new[-1][-1][1][1]
                                    + sub[i+e-1][1][1])
                                # change last cp to that of the last node in group
                ...
                if finishedAdding  == False:
                    ...
                    if i == 1:
                        ...
                        if e > 0:
                            ...
                            new[-1][-1][0]= sub[i+1][0]  # get first cp from i+1
                        ...
                        new[-1][-1][2]=sub[i+e][2]  # change last cp to that of the last node in group
        ...
        while s < nSub:
            ...
            if joinEndTo[s] == -1:
                ...
                if id2Test.size > 0:
                    ...
                    res = join_search(diff_x, diff_y, maxdist2, id2Test)  # find shortest distance if less than minimum
                    if res[0] == True:
        ...
        for id, elem in self.svg.selection.id_dict().items():
            ...
            if thisIsPath:
                ...
                new, closed = remove_duplicate_nodes(
                    elem, minlength, maxdist, maxdist2,
                    self.options.allowReverse, self.options.optionJoin)
